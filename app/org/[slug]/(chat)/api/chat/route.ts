...
export async function POST(
  ...
  try {
    ...
    if (copilotId) {
      activeCopilot = await getCopilotById(copilotId, orgId);
      if (!activeCopilot || !activeCopilot.isActive) {
        return new ChatbotError("bad_request:api").toResponse();
      }
      // Verify the user has access to this co-pilot
      const available = await getAvailableCopilots(session.user.id, orgId);
    ...
    const costLimitExceeded = await checkCostLimits(session.user.id, userType, orgId);
    ...
    if (chat) {
      ...
      titlePromise = generateTitleFromUserMessage({ message, userId: session.user.id, orgId });
    }
    ...
    let memoryContext: string | null = null;
    {
      const memories = memoryEnabled
        ? await getMemoriesByUserId(session.user.id, orgId, 50)
        : [];
      ...
      execute: async ({ writer: dataStream }) => {
        const result = streamText({
          ...
          tools: {
            ...
            updateDocument: updateDocument({ session, dataStream, selectedChatModel, orgId }),
            requestSuggestions: requestSuggestions({ session, dataStream, orgId }),
            ...
            ...(memoryEnabled && !isReasoningModel
              ? { saveMemory: saveMemory({ userId: session.user.id, orgId }) }
              : {}),
            ...mcpTools,
          },
        ...
        if (titlePromise) {
          ...
          updateChatTitleById({ chatId: id, title, orgId });
        }
      },
...
export async function DELETE(
  ...
  const deletedChat = await deleteChatById({ id, orgId });
